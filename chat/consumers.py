...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def fetch_messages(self, event):
        ...
        await self.send(text_data=json.dumps({"messages": messages, "files": files}))
    ...
    async def receive(self, text_data):
        ...
        # 메시지를 데이터베이스에 저장
        ChatMessage.objects.create(message=message, sender=self.scope["user"], room_id=self.room_id)

        # 새로운 메시지를 가져오는 이벤트를 트리거
        await self.channel_layer.group_send(self.room_group_name, {"type": "fetch_messages"})
